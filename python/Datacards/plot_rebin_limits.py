...
def get_limits_onebinning(masspoints, binsuffix, scriptsuffix, mode, plotdir, makeplot):
    ...
    for year in runyears_all:
        ...
        for cat in eventcat_all:
            ...
            if allmassfound and (makeplot or cat == "allcat"):
                limithelper.makeBrazilPlot(masspoints, limits_dict[year][cat], xtitle, text, plotname, False, year)

    return limits_dict 

def get_limits_allbinnings(masspoints, nnbins_v, hmbins_v, scriptsuffix, mode, compareplotdir):
    ...
    for nnbin in nnbins_v:
        for hmebin in hmbins_v:
            ...
            if not os.path.exists(plotdir):
                os.mkdir(plotdir)
            all_limits_dict[binsuffix] = get_limits_onebinning(masspoints, binsuffix, scriptsuffix, mode, plotdir, makesingleplot)

    #nominalbin = "nnbin10HMEv1"
    nominalbin = "nnbin20HMEv3"
    ##compare within one year, different categories
    plot_catergories = True
    ...
    nnbins_bincompare = [5,10]
    hmebins_bincompare = [1,2]
    plot_HMTbinnings = False
    if plot_HMTbinnings:
    ...
    plot_nnbinnings = False
    if plot_nnbinnings:
        for year in years_bincompare:
            for cat in cats_bincompare:
                for hmebin in hmbins_bincompare:
                    nnbins_to_limits= {}
                    for nnbin in nnbins_v:
                        binsuffix = "nnbin%sHMEv%d"%(nnbin, hmebin)
...
masspoints = [260, 300, 400]
masspoints = [500, 700, 800]
nnbins_v = [20]
hmbins_v = [3]
...
#mode = "mainSys"
mode = "lnN"
compareplotdir = "compare_limitsplots_FR2_rebin_2021106"

get_limits_allbinnings(masspoints, nnbins_v, hmbins_v, scriptsuffix, mode, compareplotdir)
