...
def runImpacts(masslist, workdir, prefix, generatescripts, runscripts):
    ...
    limitsuffix = "signalxsec1fb_tminus1_autoMCthresh10_zerobin"
...
queue
	   ...
	   for channel in channels:
		...
		script.write("\n")
		script.write("plotImpacts.py -i {prefix}_M{mass}_{ch}_impacts_{suffix}.json -o {prefix}_M{mass}_{ch}_impacts_{suffix}_zerobin &> {prefix}_M{mass}_{ch}_impacts_{suffix}_makeplot.log\n".format(mass = mass, ch = channel,  suffix =scriptsuffix, prefix=prefix))
		script.write("\n")
		script.write("cp {prefix}_M{mass}_{ch}_impacts_{suffix}_zerobin.pdf {plotdir} \n".format(mass = mass, ch = channel,  suffix =scriptsuffix, plotdir = plotdir, prefix=prefix))
...
def runCMSHIG17006Limits(masslist, workdir, generatescripts, runscripts, plotsuffix, prefix):
    ...
    scriptsuffix = "signalxsec1fb_tminus1_autoMCthresh10_zerobin"
    if generatescripts :
	for mass in masslist:
		...
		script.write("text2workspace.py {prefix}_M{mass}_{ch}.dat -m {mass} -o {prefix}_M{mass}_{ch}_combine_workspace.root -P HiggsAnalysis.CombinedLimit.DYEstimationCombineModelTao:DYDataDrivenEstimationModelInstance  &> {prefix}_M{mass}_{ch}_{suffix}_text2ws.log\n".format(mass = mass, ch = channel, prefix = prefix, suffix =scriptsuffix))
		script.write("fi\n\n")
		...
		if runscripts:
		    ...
		    batchfname = condordir+"Batch_%s_M%d_%s.cmd"%(dirname, mass, channel)
		    batchscript = open(batchfname, "write")
		    batchscript.write("""universe              = vanilla 
    executable            = {script}
    arguments             = no
    ...
    notify_user           = taohuang@email.tamu.edu
    queue
		    """.format(script = fname, suffix = dirname, mass=mass))
...
def getlimits_nncut1D(masslist, nnout, outdir_prefix):
	...
	nncutsuffix = "nnstep0p1_nncut0p%s_HME1p1"%(str(nncut)[2:])
	nncutsuffix = "nnstep0p1_nncut0p%s_HME1p15"%(str(nncut)[2:])
	nncutsuffix = "nnstep0p05_nncut0p%s_HME1p15"%(str(nncut)[2:])
	#nncutsuffix = "nnstep0p1_nncut0p%s_HME1p2"%(str(nncut)[2:])
	nncutsuffix = "nnstep0p033_nncut0p%s_HME1p15"%(str(nncut)[2:])
	#nncutsuffix = "nnstep0p2_nncut0p%s_HME1p15"%(str(nncut)[2:])
	outdir = outdir_prefix+"_"+nnout+"_"+nncutsuffix+"_limits/"
	...
	#plotsuffix = nncutsuffix+"_NN_"+nnout+"_autoMCthresh10_bugfixed" ## for linearized2D
	workdir = pwd+outdir
	print("nncutsuffix ", nncutsuffix, " outdir ", outdir)
...
#workdir = '/afs/cern.ch/work/t/tahuang/CombinedLimit/ForGithub/CMSSW_8_1_0/src/HiggsAnalysis/CombinedLimit/HH_WWbb/GGToX0ToHHTo2B2L2Nu_NNvsHME_MCstat_nnout_MTandMT2_MJJ_nnstep0p05_nncut0p0_limits/'
#plotsuffix = "bugfixed"
#makeLimitsPlots(masslist, workdir, outdir_prefix, plotsuffix)
