...
if "DoubleMuon" in jobtype:
    ...
    modules = [mht_hh(),  muonScaleRes(Runyear), jetRecalibData(Runperiod), HHbbWWProducer(False,  DYestimation = True, triggertype = "DoubleMuon", verbose=1)]
elif "DoubleEG" in jobtype:
    Runperiod = jobtype.replace("DoubleEGRun","")[:5]
    print "Dataset DoubleEG ",Runperiod
    modules = [mht_hh(),  muonScaleRes(Runyear), jetRecalibData(Runperiod), HHbbWWProducer(False,  DYestimation = True, triggertype = "DoubleEG", verbose=1)]
elif "MuonEG" in jobtype:
    Runperiod = jobtype.replace("MuonEGRun","")[:5]
    print "Dataset MuonEG ",Runperiod
    modules = [mht_hh(),  muonScaleRes(Runyear), jetRecalibData(Runperiod), HHbbWWProducer(False,  DYestimation = True, triggertype = "MuonEG", verbose=1)]
elif jobtype != "":
    print "MC samples "
    jsonfile = None
