...
#TotalLumi = 35.920
TotalLumi = IntLumi
print "Run year ",Runyear," totalLumi ", TotalLumi, " mass points ",RadionMassPoints
...
channelcuts = {
        "MuMu":{"cut":"isMuMu && ((lep2_TrgObjfilterbits&16) == 16 && (lep1_TrgObjfilterbits&16) == 16)","extraweight": 1.0,"Data":"DoubleMuon", "latex":"#mu#mu"},
        #"MuMu":{"cut":"isMuMu","extraweight": 1.0,"Data":"DoubleMuon", "latex":"#mu#mu"},
        "MuEl":{"cut":"(isMuEl || isElMu) && ((lep2_TrgObjfilterbits&32) == 32 && (lep1_TrgObjfilterbits&32) == 32) ", "extraweight": 1.0,"Data":"MuonEG", "latex":"e#mu"},
        "ElEl":{"cut":"isElEl && ((lep2_TrgObjfilterbits&17) == 17 && (lep1_TrgObjfilterbits&17) == 17)","extraweight":1.0,"Data":"DoubleEG", "latex":"ee"},}
...
def plotCutflowHist_data(outdir):
    ...
    c1.SetGridy()
    c1.SetLogy()
    tex0 = ROOT.TLatex(0.1,0.92, " #scale[1.4]{#font[61]{CMS}} Internal"+"  "*20+"%.2f fb^{-1} (13 TeV),%d"%(TotalLumi, Runyear))
    tex0.SetNDC(); tex0.SetTextSize(.045); tex0.SetTextFont(42)
    tex1 = ROOT.TLatex(0.15, 0.83, "Data, Run%d "%Runyear)
    ...
    tex0.Draw("same")
    tex1.Draw("same")
    c1.SaveAs(outdir+"Data_Run%d_cutflow.pdf"%Runyear)
    c1.SaveAs(outdir+"Data_Run%d_cutflow.C"%Runyear)
    c1.SaveAs(outdir+"Data_Run%d_cutflow.png"%Runyear)
...
def plotCutflowHist(outdir, shortname, samplename = ""):
    ...
    tex0.Draw("same")
    tex1.Draw("same")
    plotname_suffix = "_Run%d_cutflow"%(Runyear)
    if normTobin:
        plotname_suffix = "_Run%d_cutflow_normbin1"%(Runyear)
    c1.SaveAs(outdir+samplename+plotname_suffix+".pdf")
    c1.SaveAs(outdir+samplename+plotname_suffix+".png")
    c1.SaveAs(outdir+samplename+plotname_suffix+".C")
...
def plotCutflowHist_allMC(outdir, bgnames):
    ...
    c1.SetGridy()
    c1.SetLogy()
    tex0 = ROOT.TLatex(0.1,0.92, " #scale[1.4]{#font[61]{CMS}} Internal"+"  "*30+"%.2f fb^{-1} (13 TeV),%d"%(TotalLumi, Runyear))
    tex0.SetNDC(); tex0.SetTextSize(.045); tex0.SetTextFont(42)
    hs.Draw("nostackhist")
    xaxis = hs.GetHistogram().GetXaxis()
    for i, cut in enumerate(cutflows):
    ...
    legend.Draw("same")
    tex0.Draw("same")
    c1.SaveAs(outdir+"HHbbWW_backgrounds_Run%d_cutflow.pdf"%Runyear)
    c1.SaveAs(outdir+"HHbbWW_backgrounds_Run%d_cutflow.png"%Runyear)
    c1.SaveAs(outdir+"HHbbWW_backgrounds_Run%d_cutflow.C"%Runyear)
...
def DrellYanDataDriven(channel, filedict, todraw, cut, xbins, xtitle, suffix, makeDYplots, plotname):
	...
	tex1.SetNDC(); tex1.SetTextSize(.055)
        ...
        c1.SaveAs(plotname+"_"+channel+"_"+suffix+".png")
...
def histForlimits1D(bgnames, mass, todraw, cut, xbins, xtitle, suffix, outfile, plotname):
    ...
    signalname_short = 'RadionM%d'%mass; signalname_full =  full_local_samplelist[signalname_short].keys()[0]
    ...
    colors = [800-4, 820-3, 900-3, 860-3, 616-7, 432+2, 400+2]
    nominalWeight = "passTrigger*sample_weight*event_reco_weight*{totallumi}".format(totallumi = TotalLumi)
    #nominalWeight = "sample_weight*event_reco_weight*{totallumi}*lep1trgsf*lep2trgsf/ll_Tallintrgeff".format(totallumi = TotalLumi)
    #nominalWeight = "sample_weight*event_reco_weight*{totallumi}*ll_Tallintrgeff/lep1trgsf/lep2trgsf".format(totallumi = TotalLumi)
    #nominalWeight = "sample_weight*event_reco_weight*{totallumi}*ll_Tallintrgeff/lep1trgsf/lep2trgsf/event_btag_weight".format(totallumi = TotalLumi)
    #nominalWeight = "sample_weight*event_reco_weight*{totallumi}/el_hltsafeid".format(totallumi = TotalLumi)
    #nominalWeight = "sample_weight*event_reco_weight*{totallumi}/el_hltsafeid/jjbtag_heavy/jjbtag_light".format(totallumi = TotalLumi)
    print "nominalweight ",nominalWeight
	...
	print "channel ",channel," data cut ", "("+ cut + " && "+ channelcuts[channel]["cut"] +")"," yield ",hist_data.GetEntries()
	...
	for i, key in enumerate(bgnames):
	    ...
	    BGSum = BGSum + hist.Integral()
            ...
            Finalyield[channel][key] = hist.Integral() 
	    print "mass ",mass, " channel ", channel," bg ",key," rate ",hist.Integral()
            #hist_bg_all.Print("ALL")
            

        hist_bg_all.SetLineColor(0)
        hist_bg_all.SetFillStyle(errbandfillstype)
        hist_bg_all.SetFillColor(errbandfillcolor)
	...
	hs.Write()
        ...
        print "Final yield ",Finalyield[channel]
	...
	tex0 = ROOT.TLatex(0.1,0.92, " #scale[1.4]{#font[61]{CMS}} Internal"+"  "*16+"%.2f fb^{-1} (13 TeV),%d"%(TotalLumi, Runyear))
	...
	#tex.Draw("same")
        ...
        herrband.SetFillStyle(errbandfillstype)
        herrband.SetFillColor(errbandfillcolor)
        ...
        c1.SaveAs(plotname+"_Radion_"+channel+"_"+suffix+".png")
        rfile.Close()
...
def makeBackgroundshist(masspoints, variable, nbins, xtitle, outdir):
    ...
    ###create tfile
    todraw = variable
    ##DEEPCSV: Tight: > 0.8001; Medium: > 0.4941; Loose: > 0.1522
    #cut = "ll_M<76 && ((jet1_DeepCSV>0.1522 && jet2_DeepCSV>0.4941) || (jet1_DeepCSV>0.4941 && jet2_DeepCSV>0.1522) )"
    #cut = "((lep2_TrgObjfilterbits&16) == 16 && (lep1_TrgObjfilterbits&16) == 16)"
    cut = "1"
    ...
    for mass in masspoints:
        ...
        suffix = 'M%d_Tallineff_nobtagweight_passTrigger_filterbits'%mass
...
def histForlimits2D(bgnames, mass, todraw, cut, xbins, xtitle, ybins, ytitle, suffix, outfile, plotname):
	...
	hist_data.Write()
        ...
        c1.cd(1)
        hist_s.SetTitle("Signal, M=%d GeV, %d"%(mass, Runyear))
	hist_s.Draw("colz")
        ...
        c1.Update()
        c1.cd(2)
        hist_bg_all.SetTitle("SM backgrounds, %d"%Runyear)
        ...
        c1.Update()
        c1.cd(3)
        hist_data.SetTitle("Data, %d"%Runyear)
        hist_data.Draw("colz")
        ...
        histRatio.SetTitle("Data/MC, %d"%Runyear)
...
variablesdir = "HHNtuple_20190508_variablehist_TallinEff_nobtagcut_Passtrig/"
...
masspoints = RadionMassPoints
#variables = ['lep1_pt']
outcutflowdir = "HHNtuple_20190402_addSys_cutflows/"
...
def plotallkinematics():
    ...
    #no cut on ll_M
    makeBackgroundshist(masspoints, 'll_M', [60, 10.0, 250.0], "M_{ll}", variablesdir)
...
outdir = "HHbbWW_20180704_NNoutput_MjjCR/"
