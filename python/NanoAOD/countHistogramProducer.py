...
class countHistogramProducer(Module):
  ...
  def __init__(self, era, selection):
    ...
    self.histograms = {
      ...
      'CountFullWeightedUp'        : { 'bins' : 1, 'min' : 0., 'max' : 2., },
      'CountFullWeightedDown'      : { 'bins' : 1, 'min' : 0., 'max' : 2., },
  ...
  def analyze(self, event):
    ...
    if hasattr(event, self.puWeightName) and \
      ...
      puWeightUp     = getattr(event, self.puWeightName+"Up")
      puWeightDown   = getattr(event, self.puWeightName+"Down")
      ...
      countFullWeightUp =  genWeight * puWeightUp
      countFullWeightDown =  genWeight * puWeightDown

      if len(LHEPdfWeight) != self.nLHEPdfWeight:
        #raise ValueError(
	print(
          "error!!! The length of '%s' array (= %i) does not match to the expected length of %i" % \
	...
	self.nLHEScaleWeight = len(LHEScaleWeight)
      ...
      if 'histogram' in self.histograms['CountFullWeightedUp']:
        self.histograms['CountFullWeightedUp']['histogram'].Fill(1, countFullWeightUp)
      if 'histogram' in self.histograms['CountFullWeightedDown']:
        self.histograms['CountFullWeightedDown']['histogram'].Fill(1, countFullWeightDown)
	  ...
	  #print "lhe_pdf_idx ",lhe_pdf_idx," countFullWeight  ",countFullWeight," this ",countFullWeight * LHEPdfWeight[lhe_pdf_idx], " ",LHEPdfWeight[lhe_pdf_idx]
...
countHistogramCountFullWeightedUp_2016           = lambda : countHistogramProducer('2016', 'CountFullWeightedUp')
countHistogramCountFullWeightedDown_2016           = lambda : countHistogramProducer('2016', 'CountFullWeightedDown')
...
countHistogramCountFullWeightedUp_2017           = lambda : countHistogramProducer('2017', 'CountFullWeightedUp')
countHistogramCountFullWeightedDown_2017           = lambda : countHistogramProducer('2017', 'CountFullWeightedDown')
