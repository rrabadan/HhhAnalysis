...
def computeFracandEff(torun_dataset, script, inputdir, outputdir):
    for job in torun_dataset:
        os.system("python {script} -i {inputdir} -o {outputdir} -n {name}".format(script = script, inputdir = inputdir, outputdir = outputdir, name = job))
    

def plotallNtuples(torun_datasets, inputdir, plotdir):
    for ijob, job in enumerate(torun_datasets):
    	finalfile = os.path.join(inputdir, "btagging_efficiency_"+job+".root")
...
dy_datasets = []
dy_datasets.append('DYJetsToLL_M-10to50_TuneCUETP8M1_13TeV-madgraphMLM-pythia8')
dy_datasets.append('DYToLL_0J_13TeV-amcatnloFXFX-pythia8')
dy_datasets.append('DYToLL_1J_13TeV-amcatnloFXFX-pythia8')
dy_datasets.append('DYToLL_2J_13TeV-amcatnloFXFX-pythia8')
#inputdir = "/data/taohuang/HHNtuple_20180410_DYestimation/"

outputdir = "BtaggingEff_fraction/"
#computeFracandEff(dy_datasets, "computeFlavorFractionsOnBDT_SkimNano.py", inputdir, outputdir)
#computeFracandEff(dy_datasets, "", inputdir, outputdir)
...
plotallNtuples(torun_datasets, inputdir, plotdir)
