...
p=PostProcessor(outputdir, filesdata_MuMu,"1","keep_and_drop.txt",[mht_hh(), HHbbWWProducer(False, triggertype = "DoubleMuon", verbose=3)], friend =True, jsonInput = jsonfile, provenance=True)
#p=PostProcessor(outputdir, filesdata_MuEl,"1","keep_and_drop.txt",[mht_hh(), HHbbWWProducer(False, triggertype = "MuonEG", verbose=3)], friend = True, jsonInput = jsonfile, provenance=True)
#p=PostProcessor(outputdir,filesdata_ElEl,"1","keep_and_drop.txt",[mht_hh(),  HHbbWWProducer(False, triggertype = "DoubleEG", verbose=3)], friend = True, provenance=True)
